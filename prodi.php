...
<head>
  ...
  <title>Prodi | CRUD Data Mahasiswa</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <!-- bootstrap template -->
    ...
    <div class="card shadow mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <label class="h6">Data Prodi</label>
          <a href="./prodi-form.php" class="btn btn-sm btn-info">Tambah</a>
        </div>
      </div>
      <div class="card-body">
        <table class="table table-responsive-md table-striped table-hover">
          ...
          <tbody>
            <?php
            $query = "SELECT fr.*, (
              SELECT COUNT(*) FROM mahasiswa AS mhs WHERE mhs.prodi_id = fr.id
          ) AS jml_mahasiswa FROM prodi AS fr";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              ...
              $btn_edit =  '<a href="./prodi-form.php?edit=' . $row->id . '" class="btn btn-sm btn-primary">Edit</a>';
              $btn_delete =  '<a href="./prodi.php?delete=' . $row->id . '" onclick="return confirm(\'Apakah anda yakin?\')" class="btn btn-sm btn-danger">Hapus</a>';
              echo "<tr>
              <th>$counter</th>
              <td>{$row->nama}</td>
