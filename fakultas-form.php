...
if (isset($_POST['submit'])) {
  ...
  if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $query = "UPDATE fakultas SET nama='$nama', deskripsi='$deskripsi' WHERE id='$id'";
  }
  // jika tidak ada data yang di kirim di url maka data di tambah
  else {
    $query = "INSERT INTO fakultas(id, nama, deskripsi) VALUES (null, '$nama', '$deskripsi')";
  }
...
if (isset($_GET['edit'])) {
  ...
  $result = mysqli_query($conn, "SELECT * FROM fakultas WHERE id='$id'");
...
<head>
  ...
  <title><?= $title ?> Data Fakultas | CRUD Data Mahasiswa</title>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <!-- bootstrap template -->
    ...
    <div class="card shadow mt-3">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          <label class="h6"><?= $title ?> Data Fakultas</label>
          <a href="./fakultas.php" class="btn btn-sm btn-secondary">Kembali</a>
        </div>
      </div>
      <div class="card-body">
        <form method="POST">
          <div class="form-group">
            ...
            <input type="text" class="form-control" name="nama" id="nama" value="<?= $nama ?>" placeholder="Nama Fakultas" required>
          </div>
          <div class="form-group">
